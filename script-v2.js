    // Challenge GIF rotation
    const challengeGifs = [
      'THE-MENACE.gif',
      'Waschmachine.gif'
      // Weitere GIFs können hier hinzugefügt werden
    ];
    let lastChallengeGif = null; // Track last used GIF to avoid repetition
    
    // Function to get random GIF (not the same as last time)
    function getRandomChallengeGif() {
      if (challengeGifs.length === 1) {
        return challengeGifs[0];
      }
      
      let availableGifs = challengeGifs.filter(gif => gif !== lastChallengeGif);
      const randomGif = availableGifs[Math.floor(Math.random() * availableGifs.length)];
      lastChallengeGif = randomGif;
      
      console.log('Selected challenge GIF:', randomGif);
      return randomGif;
    }
    ...
    function generateScore(min, max) {
      ...
      if (availableScores.length === 0) {
        console.log('Keine Checkouts verfügbar in diesem Bereich!');
        return;
      }
    ...
    function showFeedback(isCorrect) {
      ...
      if (isCorrect) {
        ...
        // Automatisch zur nächsten Zahl nach 1300ms (in allen Modi)
        setTimeout(() => {
          if (window.challengeMode) {
            generateScore(currentRangeMin, currentRangeMax);
          } else if (window.learnModeActive) {
    ...
    function startChallenge() {
      ...
      showCountdown(() => {
        ...
        // Select random GIF for this challenge
        const randomGif = getRandomChallengeGif();
        const challengeGifImg = document.querySelector('.challenge-gif img');
        if (challengeGifImg) {
          challengeGifImg.src = randomGif;
          console.log('Challenge GIF set to:', randomGif);
        }
    ...
    function saveSettings() {
      ...
      if (window.databaseEditMode === '2darts') {
        ...
        if (invalidScoresWithData.length > 0) {
          console.log('2-Dart-Finishes nur bis Score 110! Entfernte Einträge: ' + invalidScoresWithData.join(', '));
        }
