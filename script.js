// VERSION: PINK-DEBUG-2025-01-25-v2 - Tutorial Fix
// Vollständige Checkout-Datenbank
    ...
    function showTutorialStep(stepIndex) {
      console.log('=== showTutorialStep called ===');
      ...
      console.log('Element selector:', step.element);
      ...
      setTimeout(() => {
        ...
        console.log('Trying to find element:', step.element);
        const element = document.querySelector(step.element);
        console.log('Element found:', element);
        
        if (!element) {
          console.error('Tutorial element not found:', step.element);
          console.log('All .start-btn elements:', document.querySelectorAll('.start-btn'));
          console.log('Specific element test:', document.querySelector('.start-btn-tutorial'));
          // Still show navigation buttons even if element not found
          updateTutorialNavigation(stepIndex);
          // DON'T return - try to show tooltip anyway
          // return;
        }
    ...
    function saveSettings() {
      ...
      if (window.databaseEditMode === '2darts') {
        // Remove all scores > 110 from the database (2-dart finishes only valid up to 110)
        for (let score in newDatabase) {
    ...
    function startRangeBtnPress(e) {
      ...
      rangeBtnPressStartTime = Date.now();
      
      rangeBtnPressTimer = setTimeout(() => {
        // After 0.7 seconds - long press detected
        // Double-check that the current target is still the same button
    ...
    function handleRangeBtnRelease(e) {
      ...
      const pressDuration = Date.now() - rangeBtnPressStartTime;
      ...
      // Check if long-press was triggered AND press duration was actually long enough
      // This prevents race conditions where the timer fires but release happens too quickly
      if (!rangeBtnLongPressTriggered || pressDuration < 650) {
        // Short click - select range (only if press was shorter than 650ms)
        const min = parseInt(btn.dataset.min);
        const max = parseInt(btn.dataset.max);
